...
import streamlit as st  # Sesuai tampilan UI gelap pada screenshot Anda
...
def get_chemical_database():
    base = {
        ...
        # ===== SEBAGIAN CONTOH DARI DATABASE FLAMMABLE =====
...
def get_ghs_images():
    # Mendeteksi lokasi folder tempat script ini berada secara otomatis
    base_path = os.path.dirname(__file__) if "__file__" in locals() else os.getcwd()
    assets_dir = base_path 
    
    # Menghubungkan kategori ke nama file lokal asli yang Anda upload
    return {
        "Flammable": os.path.join(assets_dir, "fkamable.png"),  # Sesuai nama file Anda (ada typo 'fkamable')
        "Corrosive": os.path.join(assets_dir, "korosif.jpg"),   # Sesuai nama file korosif.jpg
        "Oxidizer": os.path.join(assets_dir, "oksidator.jpg"),  # Sesuai nama file oksidator.jpg
        "Toxic": os.path.join(assets_dir, "toxic.jpg"),         # Sesuai nama file toxic.jpg
        "Safe": os.path.join(assets_dir, "safe.png")            # Antisipasi jika ada kategori Safe
    }

# --- ALUR UTAMA UNTUK MENAMPILKAN GAMBAR DI STREAMLIT ---

st.title("🧪 Aplikasi Visualisasi Simbol Bahaya")

# 1. Load database bahan kimia
db_bahan = get_chemical_database()

# 2. Buat dropdown pilihan menu bahan kimia di UI
pilihan_bahan = st.selectbox("Pilih Bahan Kimia:", list(db_bahan.keys()))

if pilihan_bahan:
    # 3. Cari kategori bahayanya berdasarkan bahan yang dipilih
    kategori = db_bahan.get(pilihan_bahan)
    st.subheader(f"Kategori: {kategori}")
    
    # 4. Ambil dictionary path gambar lokal
    daftar_gambar = get_ghs_images()
    path_gambar_lokal = daftar_gambar.get(kategori)
    
    # 5. Tampilkan gambar ke layar jika file fisik ditemukan di komputer
    if path_gambar_lokal and os.path.exists(path_gambar_lokal):
        gambar_tampil = Image.open(path_gambar_lokal)
        st.image(gambar_tampil, caption=f"Simbol Bahaya untuk {pilihan_bahan}", width=180)
    else:
        st.error(f"Gagal memuat gambar! File tidak ditemukan di: {path_gambar_lokal}")
